...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def cell_double_clicked(self, row, column):
        try:
            # 获取图片路径
            if self.tableWidget_info.item(row, 6) is None:
                return
    ...
    def show_info(self, result):
        try:
            ...
            def adjustFontToFit(label, text, max_point_size=20, min_point_size=8):
                font = label.font()
                for size in range(max_point_size, min_point_size - 1, -1):
                    ...
                    if metrics.width(text) <= label.width():
                        ...
                        label.setText(text)
                        return
                # 如果最小字体都放不下，仍然设置最小字体并截断
                font.setPointSize(min_point_size)
                label.setFont(font)
                elided = QFontMetrics(font).elidedText(text, Qt.ElideRight, label.width())
                label.setText(elided)

            # 自动缩放文本以适应标签
            adjustFontToFit(self.label_class, cls_name)
